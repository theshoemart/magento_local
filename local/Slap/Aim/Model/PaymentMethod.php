...
class Slap_Aim_Model_PaymentMethod extends Mage_Payment_Model_Method_Cc
{
 ...
 /**
     ...
     * @return Slap_Aim_Model_PaymentMethod
     */
   ...
   /**
    ...
    protected function _place($payment, $amount, $requestType)
    {
        ...
        switch ($result->getResponseCode()) {
			...
			$payment->setApproval($result->getApprovalCode());    
			$this->getCardsStorage($payment)->flushCards();
				...
				/* CAVV response code match conditions  */
				if($this->getConfigData('secure_3d')){
					  //$result->getCAVVResponseCode();
					  $cavvArr = explode(",",$this->getConfigData('cavv_response_code'));
						if( !(in_array($result->getCAVVResponseCode(), $cavvArr))) {
						$this->_voidCardTransactionCavv($payment,$card->getLastTransId());
						}
     ...
     * @return _voidCardTransactionCavv
     */
    protected function _voidCardTransactionCavv($payment, $authTransactionId)
    {
        ...
        switch ($result->getResponseCode()) {
            case self::RESPONSE_CODE_APPROVED:
                $exceptionMessage = Mage::helper('paygate')->__('CAVV Response: CAVV does not match.');
                break;
         } 
     ...
     *
     * @param Mage_Payment_Model_Info $payment
     * @return Slap_Aim_Model_PaymentMethod_Cards
     */
     ...
     *
     * @return Slap_Aim_Model_PaymentMethod_Request
     */
    ...
    protected function _buildRequest(Varien_Object $payment)
    {
       ...
       if($this->getConfigData('secure_3d')){
		  ...
		  if($cavv_code->getMyCustomData() != ''){
		  $request->setXCardholderAuthenticationValue($cavv_code->getMyCustomData());
		  $request->setXAuthenticationIndicator('5');
		  }
	    }
		  //$request->setXCardholderAuthenticationValue('AAACBXUVCGhgFECHAxUIAAAAAAA=');
		  //$request->setXAuthenticationIndicator('5');
     ...
     *
     * @param Slap_Aim_Model_PaymentMethod_Request $request)
     * @return Slap_Aim_Model_PaymentMethod_Result
     */
     ...
     *
     * @param Slap_Aim_Model_PaymentMethod_Result $response
     * @param Mage_Sales_Model_Order_Payment $payment
